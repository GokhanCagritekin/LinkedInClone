package com.trendyolbootcamp.linkedinclonecompany.config;
...
@Configuration
public class CompanyCouchbaseConfiguration {

    private final CouchbaseProperties couchbaseProperties;

    public CompanyCouchbaseConfiguration(CouchbaseProperties couchbaseProperties) {
        this.couchbaseProperties = couchbaseProperties;
    }

    // connectionu bean ile kurmus olduk
    @Bean
    ...
    // default mappingler sağladığı için
    @Bean
    public Collection companyCollection(Cluster couchbaseCluster) {
        couchbaseCluster.bucket(couchbaseProperties.getBucketName()).waitUntilReady(Duration.ofSeconds(100));
